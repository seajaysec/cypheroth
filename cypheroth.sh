...
declare -a queries=(
    ...
    "All computers that members of the Domain Users group can RDP to;match p=(g:Group)-[:CanRDP]->(c:Computer) where g.objectid ENDS WITH '-513' return c.name,c.domain,c.operatingsystem,c.highvalue,c.haslaps;DomainUserRDPComputers.csv"
    ...
    "Users with paths to Domain Controllers;MATCH (u:User), (g:Group), p=shortestPath((u)-[*1..]->(g)) WHERE g.objectid ENDS WITH '-512' RETURN u.name AS UserName,u.displayname AS DisplayName, length(p) AS Hops ORDER BY Hops;UsersWithPathsToDCs.csv"
    "What permissions does Everyone/Anonymous/Authenticated users/Domain users/Domain computers have;MATCH p=(m:Group)- [r:AddMember|AdminTo|AllExtendedRights|AllowedToDelegate|CanRDP|Contains|ExecuteDCOM|ForceChangePassword|GenericAll|GenericWrite|GetChanges|GetChangesAll|HasSession|Owns|ReadLAPSPassword|SQLAdmin|TrustedBy|WriteDACL|WriteOwner|AddAllowedToAct|AllowedToAct]->(t) WHERE m.objectid ENDS WITH '-513' OR m.objectid ENDS WITH '-515' OR m.objectid='S-1-5-11' OR m.objectid='S-1-1-0' OR m.objectid='S-1-5-7' RETURN m.name,TYPE(r),t.name,t.enabled,LABELS(t);interestingPermissions.csv"
    ...
    "Groups with Computers and Admins;MATCH (c:Computer)-[r:MemberOf*1..]->(g:Group) WITH g MATCH (u:User)-[r:MemberOf*1..]->(g) RETURN DISTINCT(g.name) as Name, g.domain AS Domain, g.highvalue AS HighValue, g.objectid AS SID, g.description AS Description, g.admincount AS AdminCount;GroupsWithCompsAndAdmins.csv"
    ...
    "Full Domain Property List;MATCH(d:Domain) RETURN d.name AS Name, d.domain AS Domain, d.functionallevel AS FunctionalLevel, d.highvalue AS HighValue, d.objectid AS SID;AllDomProps.csv"
    ...
    "Everyone with Local Admin;MATCH (g:Group {domain:'$DOMAIN'}) WHERE g.objectid = 'S-1-1-0' OPTIONAL MATCH (g)-[:AdminTo]->(c1) OPTIONAL MATCH (g)-[:MemberOf*1..]->(:Group)-[:AdminTo]->(c2) WITH COLLECT(c1) + COLLECT(c2) as tempVar UNWIND tempVar AS computers RETURN DISTINCT(computers.name);everyoneWithLocalAdmin.csv"
    "Authenticated Users with Local Admin;MATCH (g:Group {domain:'$DOMAIN'}) WHERE g.objectid = 'S-1-5-11' OPTIONAL MATCH (g)-[:AdminTo]->(c1) OPTIONAL MATCH (g)-[:MemberOf*1..]->(:Group)-[:AdminTo]->(c2) WITH COLLECT(c1) + COLLECT(c2) as tempVar UNWIND tempVar AS computers RETURN DISTINCT(computers.name);authUsersWithLocalAdmin.csv"
    ...
    "Objects Controlled by Everyone;MATCH (g:Group {domain:'$DOMAIN'}) WHERE g.objectid = 'S-1-1-0' OPTIONAL MATCH (g)-[{isacl:true}]->(n) OPTIONAL MATCH (g)-[:MemberOf*1..]->(:Group)-[{isacl:true}]->(m) WITH COLLECT(n) + COLLECT(m) as tempVar UNWIND tempVar AS objects RETURN DISTINCT(objects) ORDER BY objects.name ASC;ObjectsControlledByEveryone.csv"
    "Objects Controlled by Authenticated Users;MATCH (g:Group {domain:'$DOMAIN'}) WHERE g.objectid = 'S-1-5-11' OPTIONAL MATCH (g)-[{isacl:true}]->(n) OPTIONAL MATCH (g)-[:MemberOf*1..]->(:Group)-[{isacl:true}]->(m) WITH COLLECT(n) + COLLECT(m) as tempVar UNWIND tempVar AS objects RETURN DISTINCT(objects) ORDER BY objects.name ASC;ObjectsControlledByAuthenticatedUsers.csv"
    ...
    "Everyone with RDP Rights;MATCH (g:Group {domain:'$DOMAIN'}) WHERE g.objectid = 'S-1-1-0' OPTIONAL MATCH (g)-[:CanRDP]->(c1) OPTIONAL MATCH (g)-[:MemberOf*1..]->(:Group)-[:CanRDP]->(c2) WITH COLLECT(c1) + COLLECT(c2) as tempVar UNWIND tempVar AS computers RETURN DISTINCT(computers.name);EveryonewithRDPRights.csv"
    "Authenticated Users with RDP Rights;MATCH (g:Group {domain:'$DOMAIN'}) WHERE g.objectid = 'S-1-5-11' OPTIONAL MATCH (g)-[:CanRDP]->(c1) OPTIONAL MATCH (g)-[:MemberOf*1..]->(:Group)-[:CanRDP]->(c2) WITH COLLECT(c1) + COLLECT(c2) as tempVar UNWIND tempVar AS computers RETURN DISTINCT(computers.name);AuthenticatedUserswithRDPRights.csv"
    ...
    "Everyone with DCOM Rights;MATCH (g:Group {domain:'$DOMAIN'}) WHERE g.objectid = 'S-1-1-0' OPTIONAL MATCH (g)-[:ExecuteDCOM]->(c1) OPTIONAL MATCH (g)-[:MemberOf*1..]->(:Group)-[:ExecuteDCOM]->(c2) WITH COLLECT(c1) + COLLECT(c2) as tempVar UNWIND tempVar AS computers RETURN DISTINCT(computers.name);EveryonewithDCOMRights.csv"
    "Authenticated Users with DCOM Rights;MATCH (g:Group {domain:'$DOMAIN'}) WHERE g.objectid = 'S-1-5-11' OPTIONAL MATCH (g)-[:ExecuteDCOM]->(c1) OPTIONAL MATCH (g)-[:MemberOf*1..]->(:Group)-[:ExecuteDCOM]->(c2) WITH COLLECT(c1) + COLLECT(c2) as tempVar UNWIND tempVar AS computers RETURN DISTINCT(computers.name);AuthenticatedUserswithDCOMRights.csv"
    "Hops From Kerberoastable Users to DA;MATCH (u:User {domain:'$DOMAIN',hasspn:true}) MATCH (g:Group {domain:'$DOMAIN'}) WHERE g.objectid ENDS WITH '-512' AND NOT u.name STARTS WITH 'KRBTGT@' MATCH p = shortestPath((u)-[*1..]->(g)) RETURN u.name AS UserName,LENGTH(p) AS Hops ORDER BY LENGTH(p) ASC;HopsFromKerberoastableUsersToDA.csv"
    "Hops From ASREProastable Users to DA;MATCH (u:User {domain:'$DOMAIN',dontreqpreauth:true}) MATCH (g:Group {domain:'$DOMAIN'}) WHERE g.objectid ENDS WITH '-512' AND NOT u.name STARTS WITH 'KRBTGT@' MATCH p = shortestPath((u)-[*1..]->(g)) RETURN u.name AS UserName,LENGTH(p) AS Hops ORDER BY LENGTH(p) ASC;HopsFromASREPRoastableUsersToDA.csv"
