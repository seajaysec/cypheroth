...
declare -a queries=(
    ...
    "Users that are admin on 1+ machines, sorted by admin count;MATCH (U:User {domain:'$DOMAIN'})-[:MemberOf|AdminTo*1..]->(C:Computer) WITH U.name as n, COUNT(DISTINCT(C)) as c WHERE c>0 RETURN n AS UserName, c ORDER BY c DESC;UserAdminCount.csv"
