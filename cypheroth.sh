...
declare -a queries=(
    ...
    "What permissions does Everyone/Anonymous/Authenticated users/Domain users/Domain computers have;MATCH p=(m:Group)-[r:AddMember|AdminTo|AllExtendedRights|AllowedToDelegate|CanRDP|Contains|ExecuteDCOM|ForceChangePassword|GenericAll|GenericWrite|GetChanges|GetChangesAll|HasSession|Owns|ReadLAPSPassword|SQLAdmin|TrustedBy|WriteDACL|WriteOwner|AddAllowedToAct|AllowedToAct]->(t) WHERE (m.domain='$DOMAIN' OR m.name ENDS WITH '@$DOMAIN') AND (m.objectid ENDS WITH '-513' OR m.objectid ENDS WITH '-515' OR m.objectid CONTAINS 'S-1-5-11' OR m.objectid CONTAINS 'S-1-1-0' OR m.objectid CONTAINS 'S-1-5-7' OR m.name STARTS WITH 'ANONYMOUS@') RETURN m.name,TYPE(r),t.name,t.enabled,LABELS(t) ORDER BY m.name;interestingPermissions.csv"
