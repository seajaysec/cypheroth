...
declare -a queries=(
    ...
    "Number of users who can read LAPS password for each computer;MATCH (u:User)-[:MemberOf*0..]->(Group)-[:AllExtendedRights|ReadLAPSPassword]->(c:Computer) WITH c.name as computer, COUNT(DISTINCT(u)) as nb_users WHERE nb_users>0 RETURN computer, nb_users ORDER BY nb_users DESC;NbUsersCanReadLapsPerComputer.csv"
