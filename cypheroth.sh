...
declare -a queries=(
    ...
    "Groups where Everyone/Anonymous/Authenticated Users/Users/Domain Users/Domain Computers are members;MATCH p=(m:Group)-[:MemberOf*1..]->(i:Group) WHERE (m.domain='$DOMAIN' OR m.name ENDS WITH '@Z$DOMAIN') AND (m.objectid ENDS WITH '-513' OR m.objectid ENDS WITH '-515' OR m.objectid CONTAINS 'S-1-5-11' OR m.objectid CONTAINS 'S-1-5-32-545' OR m.objectid CONTAINS 'S-1-1-0' OR m.objectid CONTAINS 'S-1-5-7' OR m.name STARTS WITH 'ANONYMOUS@') RETURN m.name,i.name,LENGTH(p) as hops ORDER BY m.name;wideGroupsMemberOfOthers.csv"
